...
class ROPChainListModel(QAbstractItemModel):
    ...
    def update_rows(self):
        ...
        # Fill self.rows
        for i, addr in enumerate(self.state.chain):
            offset = (len(self.state.chain) - i - 1) * 4
            self.rows.append([offset, addr, self.disasm_at_addr(addr)])
    ...
    def disasm_at_addr(self, addr):
        if self.bv.start >= addr or addr > self.bv.end:
            return ""

        ops = []
        line = ""
        while line != "ret" and len(ops) < 3:
            line = self.bv.get_disassembly(addr)
            ops.append(re.sub(r'\s\s+', ' ', line.strip()))
            addr += self.bv.arch.get_instruction_info(self.bv[addr:addr + self.bv.arch.max_instr_length], addr).length
    ...
    def setData(self, index, value, role):
        ...
        self.state[index.row()] = new_val
        self.state.update_segment()
        self.bv.commit_undo_actions()

        self.rows[index.row()] = [old_row[0], new_val, self.disasm_at_addr(new_val)]
...
def init_ui():
    widget.register_dockwidget(ROPChainWidget, "ROPChain", Qt.RightDockWidgetArea, Qt.Vertical, False)
